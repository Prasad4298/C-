...
void InsertLast(PPNODE head, int no)
{
	...
	// travel till last node of LL and
...
void DeleteLast(PPNODE head)
{
	...
	// If LL contains more than one node then travel tell second last node and delete last node;
	...
	{
		free(*head);
		*head = NULL;	// is most important syntax
	}
	else 	// LL contains more than one node
	{
		...
		free(temp -> next);
		temp -> next = NULL;	// this is important
	}
}
...
void InsertAtPos(PPNODE head, int no, int pos)
{
	...
	if((pos < 1) || (pos > (size + 1)))		// ithe filter lavla:
	{
...
void DeleteAtPos(PPNODE head, int pos)
{
	...
	if((pos < 1) || (pos > size))		// ithe filter lavla:
	{
		printf("Position is invalid \n");
		return;
	}
...
{
	int iRet = 0;
	PNODE first = NULL;		// typedef struct node * first = NULL;
